...
+get_number/1+::
  ...
  Your function should accept either integers or floats. This involves
  going through a fair number of steps.
  ...
  * Use +Regex.match?/2+ to see if the string has a dot in it (See the following section for details on this.)
  * If there is a decimal point, use +binary_to_float/1+, otherwise use +binary_to_integer/1+ to convert it to a number.
