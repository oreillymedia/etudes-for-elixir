...
Adds the user name, server name, and pid (which is in the +from+ parameter) to the server's state. Don't allow a duplicate user name from the same server. You can use +Enum.find/3+ for this.
...
calling the +Person+ module at +server_name+ and giving it a
+:get_profile+ request.
...
+login(user_name)+::
Calls the chat room server with a {:login, user_name} request. If the
...
+Recipient list:+ are debug output. I have gotten rid of the startup
